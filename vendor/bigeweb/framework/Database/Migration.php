...
//    usort($toapply_migration, function ($a, $b) {
//        // Extract timestamps from both filenames
//        preg_match('/(\d{4}_\d{2}_\d{2}_\d{6})$/', $a, $matchA);
//        preg_match('/(\d{4}_\d{2}_\d{2}_\d{6})$/', $b, $matchB);
//
...
//    dd($toRunMigration);
    ...
    foreach ($toRunMigration as $migration) {
        ...
        $className = $namespace.DIRECTORY_SEPARATOR.$migration['name'];
