...
class BaseController
{
    ...
    public function cleanPage($pageContent)
    {
        ...
        $pattern = '/@section\(["\']description["\'],\s*(?:(["\'])(.*?)\1|(.+?))\)/';
        if (preg_match($pattern, $pageContent, $matches)) {
           $descriptionText =  isset($matches[2]) && $matches[2] !== '' ? $matches[2] : trim($matches[3]);
           $pattern = reset($matches);
        ...
        $pattern = '/@section\(["\']keywords["\'],\s*(?:(["\'])(.*?)\1|(.+?))\)/';
        if (preg_match($pattern, $pageContent, $matches)) {
           $keywords =  isset($matches[2]) && $matches[2] !== '' ? $matches[2] : trim($matches[3]);
           $pattern = reset($matches);
