...
class Validation
{
           ...
           foreach($value as $rule)
           {
               ...
               if($ruleName)
               {
                  ...
                  $errorMessage = $this->errorBag->addErrorBag($ruleName, $key);
                   ...
                   //validate required
                   if($ruleName == 'required' || $ruleName == 'require' || $ruleName == "file_required")
                   {
                       if($ruleName == "file_required")
                       {
                           $inputName = $this->getFileName($key);
                           if(!$inputName)
                           {
    ...
    public function getFileName($attribute_param)
    {
        $inputName = $this->request->file($attribute_param);
        if(strpos($attribute_param, '.*') !== false)
        {
            //means it is an array input
            //return the names in array_format
            $input_name = rtrim($attribute_param, '.*');
            $inputName = $this->request->file($input_name, []);
        }
        return $inputName;
    }
